export default {
    ...
    methods: {
        addUser() {
            if (this.saving) return;    // ✅ Bug 1 fix
            this.saving = true;
            ...
            .then(response => {
                if (!response.ok) throw new Error('Server error');  // ✅ Bug 3 fix
            ...
            .then(data => {
                if (data.success === true) {
                    ...
                    alert(data.message);   // ✅ Bug 4 fix - no redundant redirect
                }
            })
            .catch(error => {
                console.error('Error:', error);
                alert("An error occurred during registration.");  // ✅ Bug 4 fix
            })
            .finally(() => { this.saving = false; });  // ✅ Bug 1 fix
        }
    }
}